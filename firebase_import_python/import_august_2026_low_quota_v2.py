...
def as_datetime(value: Any) -> datetime:
    ...
    # IMPORTANT:
    # Firestore dapat mengembalikan DatetimeWithNanoseconds, yaitu subclass
    # datetime. Jangan memakai value.replace(...) pada object subclass ini,
    # karena pada kombinasi library tertentu object hasil replace dapat kehilangan
    # atribut internal `_nanosecond` dan gagal saat diserialisasi kembali.
    #
    # Selalu bangun datetime Python standar secara eksplisit.
    if isinstance(value, datetime):
        return datetime(
            value.year,
            value.month,
...
def normalize_for_firestore(value: Any) -> Any:
    """
    Ubah semua subclass datetime (termasuk DatetimeWithNanoseconds) menjadi
    datetime Python standar sebelum dimasukkan ke WriteBatch.

    SERVER_TIMESTAMP dan sentinel Firestore lain tidak merupakan datetime,
    sehingga dibiarkan apa adanya.
    """
    if isinstance(value, datetime):
        return datetime(
            value.year,
    ...
    if isinstance(value, dict):
        return {
            key: normalize_for_firestore(item)
            for key, item in value.items()
        }

    if isinstance(value, list):
        return [normalize_for_firestore(item) for item in value]

    if isinstance(value, tuple):
        return tuple(normalize_for_firestore(item) for item in value)
...
def load_documents(
    ...
    """
    Membaca hanya document ID yang memang dibutuhkan menggunakan get_all().
    Ini menghindari stream seluruh collection products/counters.
    """
    ...
    references = [
        database.collection(collection_name).document(document_id)
...
def load_active_batches(database) -> dict[str, dict[str, Any]]:
    """
    Hanya membaca batch berstatus active.
    Karena sistem memiliki maksimum 40 storage location, hasil query aktif
    normalnya jauh lebih kecil dibanding membaca seluruh histori batch.
    """
    query = database.collection(COL_BATCHES).where(
    ...
    for snapshot in query.stream():
        ...
        if int(data.get("remainingQty") or 0) <= 0:
            continue
        result[snapshot.id] = data
    return result


def existing_august_transactions(database) -> list[str]:
    """
    Query hanya dokumen transaction dengan createdAt pada Agustus 2026.
    Type tetap disaring lokal agar query tidak membutuhkan composite index.
    """
    period_start = datetime(EXPECTED_YEAR, EXPECTED_MONTH, 1)
    period_end = datetime(2026, 9, 1)

    query = (
        database.collection(COL_TRANSACTIONS)
        .where(filter=FieldFilter("createdAt", ">=", period_start))
        .where(filter=FieldFilter("createdAt", "<", period_end))
    )
    ...
    for snapshot in query.stream():
        ...
        if transaction_type in {"stock_in", "stock_out"}:
            found.append(snapshot.id)

    return sorted(found)


def ensure_new_batch_ids_are_free(database, batch_ids: list[str]) -> None:
    """
    Pengaman tambahan: batch code baru harus benar-benar belum ada.
    Yang diperiksa hanya ID batch hasil simulasi Agustus, bukan seluruh collection.
    """
    ...
    references = [
        database.collection(COL_BATCHES).document(batch_id)
    ...
    for snapshot in database.get_all(references):
        if snapshot.exists:
            collisions.append(snapshot.id)

    if collisions:
        preview = "\n- ".join(collisions[:10])
        raise RuntimeError(
            "Ditemukan kode batch Agustus yang sudah ada di Firestore. "
            "Import dibatalkan untuk mencegah overwrite.\n"
            f"- {preview}"
        )
...
def validate_current_state(
    ...
    """
    Validasi hanya membutuhkan batch aktif untuk FIFO dan rekonsiliasi stok.
    Batch produk lain tetap dihitung sebagai pemakai lokasi supaya allocator
    tidak memakai location yang sedang ditempati.
    """
    queues: dict[str, list[BatchState]] = {product_id: [] for product_id in products}
    occupied_locations: dict[str, str] = {}
    max_active_sequences: dict[str, int] = {product_id: 0 for product_id in products}

    for document_id, data in active_batches.items():
        product_id = str(data.get("productId") or "").strip()
        ...
        if storage_location not in LOCATIONS:
            raise RuntimeError(
                f"Batch aktif {document_id} memiliki lokasi tidak valid: {storage_location}"
            )
        if storage_location in occupied_locations:
            raise RuntimeError(
                f"Lokasi {storage_location} dipakai lebih dari satu batch aktif: "
                f"{occupied_locations[storage_location]} dan {document_id}"
            )

        occupied_locations[storage_location] = document_id
        ...
        sequence = extract_sequence(batch.batch_code)
        if sequence > max_active_sequences[product_id]:
            max_active_sequences[product_id] = sequence
    ...
    for product_id, product in products.items():
        actual_batch_stock = sum(
            batch.remaining_qty for batch in queues.get(product_id, [])
        )
    ...
    return queues, occupied_locations, max_active_sequences
...
def simulate_august(
    ...
    queues, occupied_locations, max_sequences = validate_current_state(products, active_batches)
    ...
    for row in rows:
        ...
        if row.stock_in > 0:
            ...
            if batch_code in new_batches:
                raise RuntimeError(f"Kode batch Agustus duplikat di simulasi: {batch_code}")
    ...
    return {
        ...
        "new_batch_ids": sorted(new_batches),
...
def commit_august(database, data: dict[str, Any]) -> None:
    ...
    batch.set(
        ...
        normalize_for_firestore(import_log),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Incremental import data stok Agustus 2026 ke Firestore dengan pembacaan "
            "hemat quota tanpa mengimpor ulang Januari-Juli"
        )
    )
    parser.add_argument(
        "--commit",
    ...
    print("Mode baca: LOW QUOTA + SAFE TIMESTAMP.")
    print()
    ...
    dataset_product_ids = sorted({row.product_id for row in rows})
    dataset_counter_ids = [counter_id(product_id) for product_id in dataset_product_ids]

    products = load_documents(database, COL_PRODUCTS, dataset_product_ids)
    counters = load_documents(database, COL_COUNTERS, dataset_counter_ids)
    active_batches = load_active_batches(database)

    missing_products = [
        product_id for product_id in dataset_product_ids if product_id not in products
    ]
    if missing_products:
        raise RuntimeError(
            "Produk pada Excel belum ada di collection products. Import dibatalkan:\n- "
    ...
    august_existing = existing_august_transactions(database)
    if august_existing:
    ...
    print(
        f"  Read hemat quota: products={len(products)}, "
        f"active_batches={len(active_batches)}, counters={len(counters)}, "
        "transactions_Agustus=0"
    )
    ...
    ensure_new_batch_ids_are_free(database, data["new_batch_ids"])
    print_summary(data)
    ...
    if not arguments.commit:
        ...
        print("python import_august_2026_low_quota_v2.py --commit")
        return
