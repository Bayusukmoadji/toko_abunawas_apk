...
def main():
    ...
    # Movement Agustus hanya boleh ikut rollback bila movement tersebut milik
    # batch yang memang dibuat oleh stock_in Agustus dan akan dihapus.
    unsafe_movements = {}
    safe_movements = {}
    for movement_id, data in august_movements.items():
        batch_id = str(data.get("batchId") or "").strip()
        if batch_id in result["delete_batch_ids"]:
            safe_movements[movement_id] = data
        else:
            unsafe_movements[movement_id] = data
    ...
    if unsafe_movements:
        ...
        print("===================")
        print(
            "Ada batch_movements Agustus yang menyentuh batch pra-Agustus atau "
            "batch yang tidak akan dihapus. Kasus ini perlu rollback lokasi manual."
        )
        for movement_id, data in list(unsafe_movements.items())[:20]:
            print(
    ...
    print(f"Movement Agustus dihapus  : {len(safe_movements)}")
    ...
    # Delete August movement history that belongs to August-created batches
    # being removed by this rollback.
    for movement_id in safe_movements:
        batch_write.delete(db.collection(COL_MOVEMENTS).document(movement_id))
    ...
    movements_after = load_collection(db, COL_MOVEMENTS)
    for movement_id in safe_movements:
        if movement_id in movements_after:
            raise RuntimeError(f"Batch movement Agustus masih ada setelah rollback: {movement_id}")
    ...
    print("✓ Batch movement Agustus terkait batch tersebut sudah dihapus.")
